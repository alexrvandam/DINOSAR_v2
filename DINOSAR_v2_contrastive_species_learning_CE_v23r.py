...
- Loss: Supervised Contrastive Loss (SupCon) or Triplet Loss
"""
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Forward
        # Forward
        if getattr(args, "use_classifier_head", False):
        ...
        if batch_idx % 10 == 0:
            if con_loss is not None and ce_loss is not None:
                if args.supcon_label_mode == "both" and con_loss_species is not None and con_loss_specimen is not None:
                    print(
                        f"  Batch {batch_idx}/{len(dataloader)}, "
                        f"Total: {loss.item():.4f} | "
                        f"SupCon(avg)={con_loss.item():.4f} "
                        f"[species={con_loss_species.item():.4f}, specimen={con_loss_specimen.item():.4f}] "
                        f"| CE={ce_loss.item():.4f}"
                    )
                else:
                    print(
                        f"  Batch {batch_idx}/{len(dataloader)}, "
                        f"Total: {loss.item():.4f} | SupCon: {con_loss.item():.4f} | CE: {ce_loss.item():.4f}"
                    )
            elif con_loss is not None:
                print(f"  Batch {batch_idx}/{len(dataloader)}, SupCon loss: {con_loss.item():.4f}")
            elif ce_loss is not None:
                print(f"  Batch {batch_idx}/{len(dataloader)}, CE loss: {ce_loss.item():.4f}")
